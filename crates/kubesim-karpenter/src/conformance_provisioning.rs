    ...
    BehaviorSpec {
        ...
        test: Box::new(|profile| {
            ...
            let pool = NodePool {
                name: "default".into(),
                instance_types: vec![
                    "m5.xlarge".into(),   // 4 vCPU
                    "m5.2xlarge".into(),  // 8 vCPU
                ],
            ...
            // Large batch: 2 pods × 3000m CPU (different affinity → separate batch)
            for _ in 0..2 {
            ...
            // FFD should place the large batch first. Verify we get multiple
            // instance types (heterogeneous fleet) — large batch needs m5.2xlarge,
            // small batch fits on m5.xlarge.
...
fn deletion_cost_victim_selection_spec() -> BehaviorSpec {
    ...
    BehaviorSpec {
        ...
        test: Box::new(|_profile| {
            ...
            let owner = OwnerId(1);
            // Create 3 pods with different deletion costs
            ...
            // RS with 3 desired, then scale down to 1
            ...
            // The pod with cost=100 should survive (highest cost)
...
fn drain_triggers_rs_reconcile_spec() -> BehaviorSpec {
    ...
    BehaviorSpec {
        ...
        test: Box::new(|_profile| {
            ...
            let owner = OwnerId(1);
            // Place 2 pods on the drain target
            for _ in 0..2 {
                let pod = Pod {
            ...
            // Glue handler: schedules pending pods onto available nodes
            ...
            // Cordon the drain target (as Karpenter does before draining)
            ...
            // After drain + RS reconcile, 2 pods should be running (rescheduled)
            ...
            // Pods should NOT be on the drained node
